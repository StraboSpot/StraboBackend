...
$project_rows = $db->get_results_prepared("SELECT * FROM straboexp.project WHERE userpkey = $1 ORDER BY modified_timestamp DESC", array($userpkey));
$projects = [];
foreach($project_rows as $prow){
	...
	$experiments = [];
	$experiment_rows = $db->get_results_prepared("SELECT * FROM straboexp.experiment WHERE project_pkey = $1 ORDER BY modified_timestamp DESC", array($p->pkey));
