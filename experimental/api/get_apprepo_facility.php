<?php
/**
 * File: get_apprepo_facility.php
 * Description: Returns a single facility from the apparatus repository (without apparatuses)
 *              Used when loading facility data from apparatus repository selection
 *
...
// Get facility ID from query parameter
$id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

if ($id <= 0) {
    http_response_code(400);
    echo json_encode(['error' => 'Facility ID not provided'], JSON_PRETTY_PRINT);
    exit;
}
...
try {
    ...
    if (!$row || !$row->pkey) {
        ...
        echo json_encode(['error' => 'Facility not found'], JSON_PRETTY_PRINT);
        exit;
    }
