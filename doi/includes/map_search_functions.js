...
var updateMap = function() {
	...
	if(loadedFeatures!="" && loadedFeatures!=null && loadedFeatures.features.length > 0){
		...
		//console.log("datasets from loadedFeatures:");
		...
		_.each(datasets, function(d){
			...
			if(layerStates[d.id]=="no"){
				...
				//console.log("turn that layer OFF");
			}
		...
		//console.log("all datasets layer:");
		//console.log(datasetsLayer);
...
var loadDOI = async (uuid) => {
	
	console.log(sample_vars);
	
	$('#spotswaiting').show();
	await delay(1000);
	//console.log("load doi here");
	
	$.post( "https://strabospot.org/doi/doiproject.php?u="+uuid, function( result ) {
		//console.log('Did doi project search and received:');
		console.log(result);
...
var oldgotoDataset20240516 = async (datasetid) => {
//var gotoDataset = function(datasetid){
	$('#spotswaiting').show();
	await delay(3000);
...
var expandDataset = function(dataset){
...
var downloadData = function(datatype){
	...
	if(boxeschecked){
		...
		//new
		//console.log('doisearchdownload?type='+datatype+'&dsids='+dsids+'&range='+range+'&envelope='+envelope+'&querystring='+querystring);
