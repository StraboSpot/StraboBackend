...
<?php
	foreach($exprows as $row){
		...
		$valRows = $db->get_results_prepared("select pkey,projectname,uuid,to_char(datecreated, 'Mon DD, YYYY - HH12:MI:SSPM TZ') as datecreated,experimentcount
													from straboexp.versions where uuid = $1 and userpkey = $2 order by pkey asc", array($row->uuid, $userpkey));
...
<?php
$projectrows = $db->get_results_prepared("select projectid from versions where userpkey=$1 group by projectid order by projectid desc", array($userpkey));
if(count($projectrows)==0){
	...
	foreach($projectrows as $projectrow){
		...
		$versionrows = $db->get_results_prepared("select projectname,uuid,to_char(datecreated, 'Mon DD, YYYY - HH12:MI:SSPM TZ') as datecreated,datasetcount,spotcount
													from versions where projectid = $1 and userpkey = $2 order by pkey asc", array($projectid, $userpkey));
