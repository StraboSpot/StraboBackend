...
if($_SESSION['userpkey']!=""){
	if($db){
		$doicount = $db->get_var_prepared("
			select count(*) from dois where user_pkey = $1
		", array($_SESSION['userpkey']));
...
if($userpkey != ""){
	...
	$collaborationcount = $db->get_var_prepared("select count(*) from collaborators where collaborator_user_pkey = $1 and accepted = true", array($userpkey));
...
?>
									...
									<li><a href="/TemplateWizard/">Field Template Wizard</a></li>
