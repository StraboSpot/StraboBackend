...
/**
 * Decode a PG-formatted array literal (e.g. "{Quartz,Olivine,'with quote'}")
 * back into a PHP array. PG's get_results returns array columns as the raw
 * text literal — we don't get a typed PHP array. Strip the braces and split
 * on unquoted commas; also handle PG's double-quote escaping for elements
 * with embedded commas / spaces / quotes.
 *
...
function pgParseTextArray($v) {
	...
	// Walk character-by-character respecting double-quoted elements.
	$out = array();
	$cur = '';
	$inQuote = false;
	$len = strlen($body);
	for ($i = 0; $i < $len; $i++) {
		...
		if ($inQuote) {
			if ($ch === '\\' && $i + 1 < $len) {
				// PG escapes embedded " and \ with leading backslash.
				$cur .= $body[$i + 1];
				$i++;
			} elseif ($ch === '"') {
				$inQuote = false;
	...
	// PG's "NULL" sentinel inside an array literal — drop these; they show
	// up when array_agg includes a NULL element. Our FILTER clauses already
	// remove them upstream, but be defensive.
