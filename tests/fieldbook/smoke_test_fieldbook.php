...
check('model: sample title + inplaceness as its form label (Field choice translation, D6) + ISO date', $s1['samples'][0]['title'] === 'FB-001' && in_array('5 - definitely in place', array_column($s1['samples'][0]['rows'], 'v'), true) && in_array('July 15, 2024 09:30 UTC', array_column($s1['samples'][0]['rows'], 'v'), true), json_encode($s1['samples'][0]));
